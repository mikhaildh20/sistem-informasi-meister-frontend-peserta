...
const inisialisasiDataMaterial = [
  {
    Key: null,
    "Nama Material/Standar Part": null,
    Jenis: null,
    Dimensi: null,
...
const dataJenis = [
  ...
  { Value: "Standar Part", Text: "Standar Part" },
];

export default function PermintaanPelangganAnalisa({ onChangePage, withID }) {
  const role =
    withID.FromAction === "Detail"
      ? "Detail"
      : JSON.parse(decryptId(Cookies.get("activeUser"))).role;
  ...
  const jenisMaterialRef = useRef(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (data3 === "ERROR") {
          throw new Error(
            "Terjadi kesalahan: Gagal mengambil data rincian biaya material/standar part."
          );
  ...
  const handleAdd = async (e) => {
    ...
    const result = await SweetAlert(
      "Simpan Rincian Basis Biaya Proses dan Material/Standar Part",
          ...
          if (data === "ERROR") {
            ...
            await Promise.all(
              dataMaterial.map(async (material) => {
                if (material.Key) {
                  const {
                    "Nama Material/Standar Part": NamaMaterial,
                    Jenis,
                    Dimensi,
                  ...
                  if (materialHasil === "ERROR") {
                    throw new Error(
                      "Terjadi kesalahan: Gagal menyimpan rincian biaya material/standar part."
                    );
                  }
                }
              })
            ...
            SweetAlert(
              "Sukses",
              "Data rincian basis biaya proses dan material/standar part berhasil disimpan",
              "success"
            );
  ...
  async function handleCopyProduk() {
    const result = await SweetAlert(
      "Salin Daftar Biaya Proses dan Material/Standar Part",
      "Biaya material/standar part, proses, dan biaya lainnya akan ditimpa dan disalin dari produk/jasa yang dipilih. Apakah Anda yakin ingin menyalin dari produk/jasa ini?",
      "warning",
      "Ya, saya yakin!"
    );
    ...
    if (result) {
      ...
      try {
        ...
        SweetAlert(
          "Sukses",
          "Data rincian biaya proses dan material/standar part berhasil disalin",
          "success"
        );
      } catch (error) {
        window.scrollTo(0, 0);
  ...
  function handleAddMaterial() {
    if (
      ...
      jenisMaterialRef.current.value !== "" &&
      ...
      const jenisMaterial =
        jenisMaterialRef.current.selectedOptions[0].textContent;
      ...
      namaMaterialRef.current.value = "";
      jenisMaterialRef.current.selectedIndex = 0;
      dimensiMaterialRef.current.value = "";
      jumlahMaterialRef.current.value = "";
      hargaMaterialRef.current.value = "";
  ...
  function hitungTotalBiayaMaterial() {
    let total = dataMaterial.reduce((akumulasi, val) => {
      if (val["Jenis"] === "Material") {
        return akumulasi + clearSeparator(val["Total Biaya (Rp)"]);
      }
      return akumulasi;
    }, 0);
  ...
  function hitungTotalBiayaStandarPart() {
    let total = dataMaterial.reduce((akumulasi, val) => {
      if (val["Jenis"] === "Standar Part") {
        return akumulasi + clearSeparator(val["Total Biaya (Rp)"]);
      }
      return akumulasi;
    }, 0);
  ...
  function hitungTotalBiaya() {
    let total =
      ...
      clearSeparator(hitungTotalBiayaStandarPart()) +
      ...
      <form onSubmit={handleAdd}>
        <div className="card">
          <div className="card-header bg-primary fw-medium text-white">
            {(role === "ROL50" ? "Atur" : "Detail") +
              " Basis Biaya Proses dan Material/Standar Part"}
          </div>
          <div className="card-body p-3">
            <div className="row">
                      ...
                      message={
                        <>
                          ...
                          <div>
                            ...
                            material/standar part, dan biaya lainnya yang
                            diperlukan untuk pembuatan suatu produk. Biaya-biaya
                            ini akan menjadi dasar perhitungan dalam Rencana
                            Anggaran Kegiatan (RAK), dan detail biaya yang
                            tercantum di halaman ini hanya berlaku{" "}
                      ...
                      <div className="row">
                        <div className="col-lg-4 mt-1">
                          <Label
                            ...
                            title="Salin daftar material/standar part, proses, dan biaya lainnya dari
                          produk/jasa lain:"
                          />
                        </div>
              ...
              <div className="col-lg-12">
                <div className="card mt-3">
                  <div className="card-header bg-secondary-subtle fw-bold">
                    Rincian Biaya Material/Standar Part
                  </div>
                  <div className="card-body p-4">
                    {role === "ROL50" && (
                      <div className="row">
                        <div className="col-lg-3 mb-2">
                          <Input
                            ...
                            ref={namaMaterialRef}
                            placeholder="Nama Material/Standar Part"
                          />
                        </div>
                        <div className="col-lg-2">
                          <DropDown
                            forInput="jenisMaterial"
                            label="Jenis"
                            showLabel={false}
                            arrData={dataJenis}
                            ref={jenisMaterialRef}
                          />
                        </div>
                        ...
                        <div className="col-lg-1 px-2 mb-2">
                          <Button
                            classType="success w-100"
                            iconName="add"
                            label="Simpan"
                        ...
                        <div className="col-lg-12 mb-3 mt-1">
                          <sup>
                            <em>
                              <strong>
                                * Gunakan tanda "," (koma) jika terdapat
                                desimal. Contoh: 10,4
                              </strong>
                            </em>
                          </sup>
                  ...
                  <div className="card-footer">
                    <div className="float-end mx-2 text-end">
                      <span className="fw-bold">
                        Total Biaya Material :
                        <span className="ms-3">
                          {hitungTotalBiayaMaterial()}
                        </span>
                      </span>
                      <br />
                      <span className="fw-bold">
                        Total Biaya Standar Part :
                        <span className="ms-3">
                          {hitungTotalBiayaStandarPart()}
                        </span>
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="card-body p-4">
                    {role === "ROL50" && (
                      <div className="row">
                        ...
                        <div className="col-lg-3 mb-2">
                          <div
                            className="px-3 border border-1 rounded-2"
        ...
        <div className="float-end my-4 mx-1">
          <Button
            ...
            label={role === "ROL50" ? "BATAL" : "KEMBALI"}
            onClick={() =>
              onChangePage(
                role === "ROL50" ? "edit" : "detail",
                withID.IDPermintaan
              )
            }
          />
