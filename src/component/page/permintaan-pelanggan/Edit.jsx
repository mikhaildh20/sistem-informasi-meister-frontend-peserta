    ...
    if (
      ...
      if (existingProdukIndex !== -1) {
        ...
        const produkBaru = {
          ...
          Spesifikasi: new DOMParser()
            .parseFromString(
              listProduk.find((val) => val.Value === parseInt(idProduk))
                .Spesifikasi,
              "text/html"
            )
            .body.textContent.substring(3),
      ...
      if (result) {
        ...
        if (data === "ERROR" || data.length === 0) {
          ...
          SweetAlert(
            ...
            "Permintaan produk/jasa ini tidak dapat disetujui karena Anda belum membuat basis biaya proses atau material/standar part yang dibutuhkan",
            "error"
          );
        } else {
          SweetAlert(
  ...
  async function handleLoadDetailProduk() {
    ...
    if (data === "ERROR") {
      ...
      const formattedData = data.map((value) => ({
        ...value,
        Spesifikasi: new DOMParser()
          .parseFromString(value["Spesifikasi"], "text/html")
          .body.textContent.substring(3),
                ...
                ...(role === "ROL50"
                  ? [
                      "Upload",
                      {
                        IconName: "calculator",
                        Title:
                          "Atur Basis Biaya Proses dan Material/Standar Part",
                        Function: () => {
                  ...
                  : [
                      {
                        IconName: "calculator-bill",
                        Title:
                          "Lihat Basis Biaya Proses dan Material/Standar Part",
                        Function: () => {
                          onChangePage("analisa", {
                            IDPermintaan: withID,
                            IDProduk: value["Key"],
